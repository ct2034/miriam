...
from roadmaps.var_odrm_torch.var_odrm_torch import draw_graph, make_graph_and_flann
...
if __name__ == "__main__":
    ...
    map_np = movingai_read_mapfile(
        f"roadmaps/odrm/odrm_eval/maps/{map_name}.map"
    ).astype(np.bool8)
    ...
    nx.set_node_attributes(
        g_dhc,
        {
            i: ((pos_grid[i][0] + 0.5) / grid_size, (pos_grid[i][1] + 0.5) / grid_size)
    ...
    width_rd = 256
    poses_rd = pkl.load(open("learn/reaction_diffusion/rd4mapf-rms-poses.pkl", "rb"))
    ...
    g_mo = nx.read_gpickle(f"multi_optim/results/mapf_benchm_{map_name}_graph.gpickle")
    _, flann_mo = make_graph_and_flann(
        pos=torch.Tensor([g_mo.nodes[n][POS] for n in g_mo.nodes]),
        map_img=map_img,
