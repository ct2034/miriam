...
def get_total_len(starts_coords, goals_coords, paths_bvc):
    ...
    for i_a, path in enumerate(paths_bvc):
        # before start
        total_lenght += float(
            np.linalg.norm(np.array(starts_coords[i_a]) - paths_bvc[i_a][0])
        )
        # path itself
        ...
        for pos in path.tolist():
            ...
            if prev_pos is not None:
                total_lenght += float(
                    np.linalg.norm(np.array(pos) - np.array(prev_pos))
                )
            prev_pos = pos
            # after goal
        total_lenght += float(
            np.linalg.norm(np.array(goals_coords[i_a]) - paths_bvc[i_a][-1])
        )
    return total_lenght
    ...
    for i_na, n_agents in enumerate(n_agents_s):
        for i_e in range(n_eval):
            ...
            agents = to_agent_objects(
                g_our, starts_our.tolist(), goals_our.tolist(), radius=radius, rng=rng
            )
            ...
            # eval bvc
            res_bvc = plan_bvc(
                map_img, starts_coords.tolist(), goals_coords.tolist(), radius=radius
            )
...
def plot(figure_folder: str, results_name: str):
    ...
    for i_na, n_agents in enumerate(n_agents_s):
        ...
        axs[i_na].bar(
            xs, lens_our[i_na] / n_agents, width, color=colors[0], alpha=1, label="ORDP"
        )
        xs += width
        axs[i_na].bar(
            xs, lens_bvc[i_na] / n_agents, width, color=colors[1], alpha=1, label="bvc"
        )
    ...
    data_lenghts = [
        [list() for _ in range(len(n_agents_s))] for _ in range(2)
    ]  # type: List[List[List[float]]]
    for i_na, n_agents in enumerate(n_agents_s):
        data_success[0][i_na] = np.count_nonzero(lens_our[i_na, :] != 0) / n_samples
        data_success[1][i_na] = np.count_nonzero(lens_bvc[i_na, :] != 0) / n_samples
    ...
    lenghts_min_max = [
        min(
            min([min(x) for x in data_lenghts[0] if len(x) > 0]),
            min([min(x) for x in data_lenghts[1] if len(x) > 0]),
        ),
        max(
            max([max(x) for x in data_lenghts[0] if len(x) > 0]),
    ...
    width = 0.8
    axs[0].bar(
        np.array(n_agents_s) - width / 2, data_success[0], width=width, label="ORDP"
    )
    axs[0].bar(
        np.array(n_agents_s) + width / 2, data_success[1], width=width, label="BVC"
    )
    ...
    elemtns = axs[1].boxplot(
        data_lenghts[0], positions=np.array(n_agents_s) - width / 2, widths=width
    )
    ...
    elemtns = axs[1].boxplot(
        data_lenghts[1], positions=np.array(n_agents_s) + width / 2, widths=width
    )
    for el in elemtns["boxes"]:
    ...
    axs[1].set_ylim(lenghts_min_max[0] - 0.1, lenghts_min_max[1] + 0.1)
...
def plot_multiple_prefixes(figure_folder: str, prefixes: List[str]):
    ...
    for i_r, prefix in enumerate(prefixes):
        yaml_data = yaml.load(
            open(f"{figure_folder}/{prefix}_lens.yaml"), Loader=yaml.SafeLoader
        )
        ...
        yaml_our = yaml.load(
            open(f"{figure_folder}/../{prefix}_stats.yaml"), Loader=yaml.SafeLoader
        )
        ...
        # 0:our, 1:bvc
        data_success = [
            [0 for _ in range(len(n_agents_s))] for _ in range(2)
        ]  # List[List[int]]
        data_lenghts = [
            [list() for _ in range(len(n_agents_s))] for _ in range(2)
        ...
        for i_na, n_agents in enumerate(n_agents_s):
            ...
            for i_e in range(n_samples):
                if both_successfull[i_na, i_e]:
                    data_lenghts[0][i_na].append(lens_our[i_na, i_e] / n_agents)
                    data_lenghts[1][i_na].append(lens_bvc[i_na, i_e] / n_agents)
        lenghts_min_max = [
            min(
                min([min(x) for x in data_lenghts[0] if len(x) > 0]),
                min([min(x) for x in data_lenghts[1] if len(x) > 0]),
        ...
        for el in elemtns["fliers"]:
            el.set_markeredgecolor(colors[1])
        axs[i_r, 1].plot(0, 99, color=colors[0], label="ORDP")  # for legend
        axs[i_r, 1].plot(0, 99, color=colors[1], label="BVC")  # for legend
        axs[i_r, 1].set_xlim(1, max(n_agents_s) + 1)
        axs[i_r, 1].set_ylim(lenghts_min_max[0] - 0.1, lenghts_min_max[1] + 0.1)
...
if __name__ == "__main__":
    ...
    prefixes: List[str] = ["large", "large_map_name_c", "large_map_name_z"]
