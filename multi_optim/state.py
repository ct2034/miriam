...
from sim.decentralized.policy import (
...
class ScenarioState:
    ...
    def __init__(self, graph, starts, goals, model, radius) -> None:
        ...
        self.finished = False
        self.agents = to_agent_objects(graph, starts, goals, radius=radius)
        self.model = model
    ...
    def observe(self) -> Optional[Dict[int, OBSERVATION]]:
        ...
        for i_a in self.is_agents_to_consider:
            observations[i_a] = agents_to_data(self.agents, i_a)
        return observations
    ...
    def step(self, actions: Dict[int, ACTION]):
        ...
        for i_a, a in enumerate(self.agents):
            if i_a in actions.keys():
                a.policy = ValueThenRaisingPolicy(a, actions[i_a])
            else:
                a.policy = RaisingPolicy(a)
            a.start = a.pos
            a.back_to_the_start()
...
def make_a_state_with_an_upcoming_decision(
    graph, n_agents, model, radius: float, rng
) -> ScenarioState:
    useful = False
    state = None
    while not useful:
