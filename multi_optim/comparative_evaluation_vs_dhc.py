...
def gridmap_from_origin_and_size(origin: np.ndarray, size: float, n_rows_cols: int):
    """
...
def pos_s_to_gridmap(map_img, n_rows_cols, pos_s_grid):
    ...
    nodes_out = []
    edgelist_out = []
    gridmap = np.full((n_rows_cols, n_rows_cols), OBSTACLE, dtype=np.int8)
    coords_from_node = []
        ...
        if is_coord_free(map_img, pos):
            ...
            if n > 0:
                if check_edge(pos_s_grid, map_img, n, n - 1):
                    edgelist_out.append((nodes_out.index(n), nodes_out.index(n - 1)))
            # check edge up
            if n >= n_rows_cols:
                if check_edge(pos_s_grid, map_img, n, n - n_rows_cols):
                    edgelist_out.append(
                        (nodes_out.index(n), nodes_out.index(n - n_rows_cols))
                    )
...
def make_dhcmap_with_n_nodes_on_img(n_nodes, map_img, rng: Random):
    ...
    while n_nodes_free < n_nodes:
        ...
        origin = np.array([rng.uniform(0, 1.0 - size), rng.uniform(0, 1.0 - size)])
...
def make_dhcmap_with_edge_len_on_img(edge_len: float, map_img: MAP_IMG):
    n_rows_cols = int(floor(1.0 / edge_len)) + 1
...
def get_total_len(
    ...
    for i_a, path in enumerate(paths_dhc_by_nodes):
        # before start
        total_len += float(
            np.linalg.norm(np.array(points[i_a]) - pos_dhc_np[starts_dhc_by_nodes[i_a]])
        )
        # path itself
        prev_pos = None  # type: Optional[np.ndarray]
        assert isinstance(path, np.ndarray)
        for pos in path.tolist():
            coord_pos = pos_dhc_np[coords_from_node_by_nodes.index(tuple(pos))]
            if prev_pos is not None:
                total_len += float(np.linalg.norm(coord_pos - prev_pos))
            prev_pos = coord_pos
        # after goal
        total_len += float(
            np.linalg.norm(
                np.array(points[i_a + n_agents]) - pos_dhc_np[goals_dhc_by_nodes[i_a]]
            )
        )
    return total_len


def get_total_len_in_grid(
    paths: np.ndarray,
    grid_width: int,
    points: np.ndarray,
):
    # length of an edge in the grid
    edge_len: float = 1.0 / (grid_width + 1)
    total_len = 0.0
    ...
    for i_a, path in enumerate(paths):
        ...
        total_len += float(
            np.linalg.norm(
                np.array(points[i_a])
                - (path[0] + np.array([0.5, 0.5])) * edge_len  # actual start
            )
        )  # start in grid
        # path itself
        prev_pos = None  # type: Optional[np.ndarray]
        ...
        total_len += float(
            np.linalg.norm(
                ...
                - (path[-1] + np.array([0.5, 0.5])) * edge_len  # actual goal
            )
        )  # goal in grid
    return total_len
...
def eval(logger, results_name, base_folder, figure_folder, n_agents_s, n_eval, radius):
    ...
    avg_edge_len_our = float(
        np.mean(
            np.linalg.norm(
                [pos_our_np[a] - pos_our_np[b] for (a, b) in g_our.edges], axis=1
            )
        )
    )
    logger.info(f"{n_nodes=}")
    ...
    if map_fname.endswith(".png"):
        ...
        (
            pos_dhc_by_nodes,
            edgelist_dhc_by_nodes,
            gridmap_by_nodes,
            coords_from_node_by_nodes,
        ) = make_dhcmap_with_n_nodes_on_img(n_nodes, map_img, rng)
        g_dhc_by_nodes = nx.from_edgelist(edgelist_dhc_by_nodes)
        assert isinstance(g_dhc_by_nodes, nx.Graph)
        pos_dhc_by_nodes_dict = {
            i: pos_dhc_by_nodes[i, :] for i in range(pos_dhc_by_nodes.shape[0])
        }
        nx.set_node_attributes(g_dhc_by_nodes, pos_dhc_by_nodes_dict, POS)
        pos_dhc_by_nodes_np = np.array(
            [pos_dhc_by_nodes[i] for i in range(n_nodes)], dtype=np.float32
        )

        # make gridmap with same edge length
        (
            pos_dhc_by_edge_len,
            edgelist_dhc_by_edge_len,
            gridmap_by_edge_len,
            coords_from_node_by_edge_len,
        ) = make_dhcmap_with_edge_len_on_img(avg_edge_len_our, map_img)
        ...
        pos_dhc_by_edge_len_dict = {
            i: pos_dhc_by_edge_len[i, :] for i in range(n_nodes_dhc_by_edge_len)
        }
        nx.set_node_attributes(g_dhc_by_edge_len, pos_dhc_by_edge_len_dict, POS)
        pos_dhc_by_edge_len_np = np.array(
            [pos_dhc_by_edge_len[i] for i in range(n_nodes_dhc_by_edge_len)],
            dtype=np.float32,
        )
        ...
        f.savefig(f"{figure_folder}/{results_name}_dhcmap_by_edge_len.png")
        plt.close(f)
        ...
        for i_na, n_agents in enumerate(n_agents_s):
            for i_e in range(n_eval):
                ...
                while not unique_starts_and_goals:
                    points = sample_points(n_agents * 2, map_img, rng).detach().numpy()
                    ...
                    unique_starts_and_goals = len(set(nn_our)) == 2 * n_agents
                ...
                if agents is not None:
                    for agent in agents:
                        agent.policy = LearnedPolicy(agent, policy_nn)
                    ...
                    if res_our[IDX_SUCCESS]:
                        ...
                        for i_a in range(n_agents):
                            # before start
                            total_lenght_our += float(
                                np.linalg.norm(
                                    np.array(points[i_a]) - pos_our_np[starts_our[i_a]]
                                )
                            )  # type: ignore
                            # after goal
                            total_lenght_our += float(
                                np.linalg.norm(
                                    np.array(points[i_a + n_agents])
                                    - np.array(
                                        pos_our[goals_our[i_a]], dtype=np.float32
                                    )
                                )
                            )  # type: ignore
                ...
                res_dhc_by_edge_len = dhc_eval(
                    ...
                    np.array(
                        [coords_from_node_by_edge_len[n] for n in goals_dhc_by_edge_len]
                    ),
                )
                ...
                if res_dhc_by_edge_len != INVALID:
                    ...
                    total_lenght_dhc_by_edge_len = get_total_len(
                        ...
                        starts_dhc_by_edge_len,
                        goals_dhc_by_edge_len,
                        paths_dhc_by_edge_len,
                    )
                ...
                if i_e == 0:
                    ...
                    if (
                        total_lenght_our is not None
                        and total_lenght_dhc_by_nodes is not None
                        and total_lenght_dhc_by_edge_len is not None
                    ):
                        f_our, ax_our = plt.subplots(figsize=(4.5, 4.5))
                        ...
                        for i_a in range(n_agents):
                            ...
                            for pos in paths_dhc_by_edge_len[i_a]:
                                coordpaths_dhc_by_edge_len.append(
                                    pos_dhc_by_edge_len_np[
                                        coords_from_node_by_edge_len.index(tuple(pos))
                                    ]
                                )
                            # goal
        ...
        for i_na, n_agents in enumerate(n_agents_s):
            for i_e in range(n_eval):
                ...
                assert map_grid.shape[0] == map_grid.shape[1], "map must be square"
                ...
                res_dhc = dhc_eval(map_grid, starts_dhc, goals_dhc)
                    ...
                    if total_lenght_our is not None and total_lenght_dhc is not None:
                        ...
                        for i_a in range(n_agents):
                            ...
                            for pos in paths_dhc[i_a]:
                                coordpaths_dhc.append(pos_dhc_dict[tuple(pos)])
                            # goal
                            coordpaths_our.append(points[i_a + n_agents])
                            ...
                            ax_dhc.plot(
                                [
                                    coordpaths_dhc[i][0]
                                    for i in range(len(coordpaths_dhc))
                                ],
                                [
                                    coordpaths_dhc[i][1]
                                    for i in range(len(coordpaths_dhc))
...
def plot(figure_folder: str, results_name: str):
    ...
    for i_na, n_agents in enumerate(n_agents_s):
        ...
        abs_lengths_min_max = [
            min(min(abs_lengths_our[i_na]), abs_lengths_min_max[0]),
            max(
                max(abs_lengths_our[i_na]),
    ...
    width = 1.6
    # lengths
    ax.boxplot([0, 1], positions=[-20], widths=width)
    elemtns = ax.boxplot(
        abs_lengths_our,
    ...
    ax.plot(0, 99, color=colors[1], label="ORDP")  # for legend
    ax.plot(0, 99, color=colors[2], label="DHC")  # for legend
    ax.set_xlim(min(n_agents_s) - width - 0.5, max(n_agents_s) + width + 0.5)
    ax.set_ylim(abs_lengths_min_max[0] - 0.1, abs_lengths_min_max[1] + 0.1)
...
if __name__ == "__main__":
    ...
    n_eval: int = 50
    radius: float = 1.0 / 32 / 2

    eval(logger, results_name, base_folder, figure_folder, n_agents_s, n_eval, radius)
