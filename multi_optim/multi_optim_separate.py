...
def plot(save_folder: str, prefix_s: List[str]):
    ...
    for prefix in prefix_s:
        ...
        both_stats_per_prefix[prefix] = yaml.load(
            open(f"{save_folder}/{prefix}_stats.yaml", "r"), yaml.SafeLoader
        )
        stats_per_prefix[prefix] = []
        stats_per_prefix[prefix].append(
            yaml.load(
        ...
        for s in stats_per_prefix[prefix]:
            available_n_agents_policy_regret = list(
                filter(lambda x: x.startswith("policy_regret_"), s.keys())
            )
            available_n_agents_general_success = list(
                filter(lambda x: x.startswith("general_success_"), s.keys())
            )
            available_n_agents_policy_regret = map(
                int, map(lambda x: x.split("_")[-1], available_n_agents_policy_regret)
            )
            available_n_agents_general_success = map(
                int, map(lambda x: x.split("_")[-1], available_n_agents_general_success)
            )
            n_agents = min(
                n_agents,
    ...
    prefixes_sorted_by_n_nodes = sorted(prefix_s, key=lambda x: n_nodes_per_prefix[x])
    ...
    for i_m, metric in enumerate(metrics):
        ...
        for i_p, prefix in enumerate(prefixes_sorted_by_n_nodes):
            ...
            for i_s, stat in enumerate(stats_per_prefix[prefix]):
                ...
                elements = axs[i_m].boxplot(
                    [data],
                    positions=[i_p + (i_s - 0.5) * width / len(titles)],
        ...
        pretty_name = " ".join(map(lambda x: x.capitalize(), metric.split("_")))
        axs[i_m].plot(-5, 0, color=colors[2], label="roadmap only - both")  # for legend
        axs[i_m].plot(-5, 0, color=colors[3], label="policy only - both")  # for legend
        axs[i_m].set_xlim(-0.5, len(n_agents_per_prefix) - 0.5)
...
if __name__ == "__main__":
    ...
    for prefix in prefix_s:
        run_optimization_sep(
            **configs_all_maps[prefix], pool=pool, save_folder=save_folder
        )
