...
def plot_env_agents(environent: np.ndarray, agents: np.ndarray):  # pragma: no cover
    ...
    for i_x in range(environent.shape[0] - 1):
        ax.plot(
            [i_x + 0.5] * 2,
            [-0.5, environent.shape[1] - 0.5],
            color="dimgrey",
            linewidth=0.5,
        )
    for i_y in range(environent.shape[1] - 1):
        ax.plot(
            [-0.5, environent.shape[0] - 0.5],
            [i_y + 0.5] * 2,
            color="dimgrey",
            linewidth=0.5,
        )
    ...
    for i_a, a in enumerate(agents):
        ...
        # path
        (line,) = ax.plot(
            np.array(a.path)[:, 0] + d_a,
            np.array(a.path)[:, 1] + d_a,
            color=colormap[i_a],
        )
...
def plot_evaluations(
    evaluations: Dict[PolicyType, np.ndarray],
    evaluation_names: List[str],
    subtract_for_policy=None,
):  # pragma: no cover
    ...
    if subtract_for_policy is not None:
        policies = [p for p in evaluations.keys() if p != subtract_for_policy]
    else:
    ...
    for i_p, policy in enumerate(policies):
        if subtract_for_policy is not None:
            data[i_p, :, :] = evaluations[policy] - evaluations[subtract_for_policy]
        else:
        ...
        all_successfull = np.logical_and(
            all_successfull, data[i_p, i_success, :]
        )  # type: ignore
    ...
    for i_e, evaluation_name in enumerate(evaluation_names):
        ...
        try:
            parts = ax.violinplot(
                np.transpose(data[:, i_e, run_choice]), showmeans=True
            )
