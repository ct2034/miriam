...
def initialize_environment_random_fill(
    size: int, fill: float, rng: Random = Random(0)
) -> np.ndarray:
    ...
    to_fill = rng.sample(list(itertools.product(range(size), repeat=2)), k=n_to_fill)
    for cell in to_fill:
        environment[cell] = 1
    return environment
...
def initialize_new_agent(
    ...
    if tight_placement:  # starts can be at other goals
        ...
        assert len(no_obstacle_nor_agent[0]) > 0, "Possible poses should be left"
        assert len(no_obstacle_nor_goal[0]) > 0, "Possible poses should be left"
        ...
        no_obstacle_nor_agent_or_goal = np.where(gridmap_with_agents_and_goals == 0)
        assert (
            len(no_obstacle_nor_agent_or_goal[0]) > 0
        ), "Possible poses should be left"
        pos = rng.choice(np.transpose(no_obstacle_nor_agent_or_goal))  # type: ignore
        a = Agent(gridmap, pos, policy, rng=rng)  # we have the agent
...
def initialize_agents(
    ...
    for _ in range(n_agents):
        agent = initialize_new_agent(env, agents, policy, tight_placement, rng)
        if agent is None:
            return None
        agents.append(agent)
...
def will_scenario_collide_and_get_paths(env, starts, goals, ignore_finished_agents):
    ...
    return will_agents_collide(
        to_agent_objects(env, starts, goals), ignore_finished_agents
    )
...
def sample_and_run_a_scenario(
    size, n_agents, policy, plot, rng: Random, iterator
) -> SCENARIO_RESULT:
    env = initialize_environment_random_fill(size, 0.1, rng)
...
def run_a_scenario(
    ...
    if paths_out is None:
        ...
        assert (
            isinstance(paths_out, list) and len(paths_out) == n_agents
    ...
    try:
        ...
        while not are_all_agents_at_their_goals(agents):
            ...
            time_slice, space_slice = get_iterator_fun(iterator)(
                agents, ignore_finished_agents
            )
            ...
            logger.debug(f"t:{max(time_progress)} finished: {finished}/{n_agents}")
            t = len(paths_out[0]) - 1
        ...
        if isinstance(e, SimIterationException):
            ...
            if isinstance(e, pause_on):  # type: ignore
                ...
                return check_time_evaluation(time_progress, space_progress) + (e,)
        else:
            raise e
    return check_time_evaluation(time_progress, space_progress) + (successful,)
...
def evaluate_policies(size=10, n_agents=10, runs=100, plot_eval=True):
    ...
    # policies = PolicyType
    policies = [PolicyType.RANDOM, PolicyType.LEARNED, PolicyType.OPTIMAL]
