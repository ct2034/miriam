...
def GMM_sklearn(x, weights=None, means=None, covariances=None):
    ...
    model.fit(x)
    print("\nscikit learn:\n\tphi: %s\n\tmu_0: %s\n\tmu_1: %s\n\tsigma_0: %s\n\tsigma_1: %s"  # nopep8
          % (model.weights_[1], model.means_[0, :], model.means_[1, :], model.covariances_[0, :], model.covariances_[1, :]))  # nopep8
...
def learn_params(x_labeled, y_labeled):
    ...
    mu0 = np.sum(x_labeled[y_labeled == 0], axis=0) / \
        x_labeled[y_labeled == 0].shape[0]
    mu1 = np.sum(x_labeled[y_labeled == 1], axis=0) / \
        x_labeled[y_labeled == 1].shape[0]
    sigma0 = np.cov(x_labeled[y_labeled == 0].T, bias=True)
    sigma1 = np.cov(x_labeled[y_labeled == 1].T, bias=True)
    return {'phi': phi, 'mu0': mu0, 'mu1': mu1, 'sigma0': sigma0, 'sigma1': sigma1}  # nopep8
...
def e_step(x, params):
    ...
    log_p_y_x = np.log([1-params["phi"], params["phi"]])[np.newaxis, ...] + \
        np.log([stats.multivariate_normal(params["mu0"], params["sigma0"]).pdf(x),  # nopep8
                stats.multivariate_normal(params["mu1"], params["sigma1"]).pdf(x)]).T  # nopep8
...
def m_step(x, params):
    ...
    params = {'phi': phi, 'mu0': mu0, 'mu1': mu1,
...
def run_em(x, params):
    ...
    while True:
        ...
        if len(avg_loglikelihoods) > 2 and abs(avg_loglikelihoods[-1] - avg_loglikelihoods[-2]) < 0.0001:  # nopep8
            break
        params = m_step(x_unlabeled, params)
    print("\tphi: %s\n\tmu_0: %s\n\tmu_1: %s\n\tsigma_0: %s\n\tsigma_1: %s"
          % (params['phi'], params['mu0'], params['mu1'], params['sigma0'], params['sigma1']))  # nopep8
...
if __name__ == '__main__':
    ...
    unsupervised_forecastsforecasts, unsupervised_posterior, unsupervised_loglikelihoods = run_em(  # nopep8
    ...
    semisupervised_forecasts, semisupervised_posterior, semisupervised_loglikelihoods = run_em(  # nopep8
    ...
    output_df = pd.DataFrame({'semisupervised_forecasts': semisupervised_forecasts,  # nopep8
    ...
    print("\n%s%% of forecasts matched." % (
        output_df[output_df["semisupervised_forecasts"] == output_df["sklearn_forecasts"]].shape[0] / output_df.shape[0] * 100))  # nopep8
