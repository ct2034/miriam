...
def find_radius(point_poses, bitmap):
    ...
    for i_p, point_pose in enumerate(point_poses):
        ...
        for x, y in product(range(c_x - 10, c_x + 10), range(c_y - 10, c_y + 10)):
            if x < 0 or y < 0 or x >= bitmap.shape[0] or y >= bitmap.shape[0]:
                continue
            if is_free(bitmap, x, y, c_x, c_y):
                radiuss[i_p] = max(
                    radiuss[i_p],
                    np.linalg.norm(np.array([x, y]) - point_pose * bitmap.shape[0]),
                )
...
def poses_to_reaction_diffusion(
    ...
    A_min_max: Tuple[float, float],
    B_min_max: Tuple[float, float],
    width: int,
    radius: float,
    ...
    the `point_poses`."""
    A_bg = A_min_max[1]
    A_fg = A_min_max[0] + 0.3
    B_bg = B_min_max[0]
    B_fg = B_min_max[1] - 0.1
...
def sim(delta_t, N, N_simulation_steps, A_bg, B_bg, experiments, mask):
...
def processing_back_and_forth(mask, experiments):
    ...
    for i, (experiment, data) in enumerate(d.items()):
        ...
        # display poses
        plot_bitmap_and_poses(axes[0, i], bitmap, point_poses, title=experiment)

        # make a graph from that
        g, _ = make_graph_and_flann(
        ...
        pos_dict = {
            i: [pos[i][1] * mask.shape[0], pos[i][0] * mask.shape[0]] for i in g.nodes()
        }
        for e in g.edges():
            a, b = e
            if a == b:
        ...
        bitmap = reaction_difussion_to_bitmap(B)
        point_poses = bitmap_to_point_poses(bitmap)
        plot_bitmap_and_poses(axes[2, i], bitmap, point_poses, title=experiment)
        ...
        bitmap = reaction_difussion_to_bitmap(B)
        point_poses = bitmap_to_point_poses(bitmap)
        plot_bitmap_and_poses(axes[3, i], bitmap, point_poses, title=experiment)
        results[experiment]["A2"] = A.copy()
        results[experiment]["B2"] = B.copy()

    pkl.dump(results, open("learn/reaction_diffusion/rd4mapf-rms-processed.pkl", "wb"))
    pkl.dump(
        poses_results, open("learn/reaction_diffusion/rd4mapf-rms-poses.pkl", "wb")
    )
    fig.tight_layout()
...
if __name__ == "__main__":
    ...
    map_np = movingai_read_mapfile(
        "roadmaps/odrm/odrm_eval/maps/random-32-32-10.map"
    ).astype(np.bool8)
