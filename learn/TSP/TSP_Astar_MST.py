...
T = G.nodes()
list.sort(T)
g = [-1 for i in range(len(T))]  # g[]: the path length from root node to the specific
        ...
        for neighbor in neighbors:
            ...
            if visit_status[neighbor_index] == -1:
                ...
                if DEBUG_verbose:
                    ...
                    (
                        "g:"
                        + str(g[neighbor_index])
                        + "\t h:"
                        + str(h)
                        + "\tf:"
                        + str(f[neighbor_index])
                    )
...
g[int(T[0])] = g[int(current_node)] + G[current_node][T[0]]["weight"]
