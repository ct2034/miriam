...
class TestGenerators(unittest.TestCase):
    ...
    def test_generate_like_sim_decentralized_determinism(self):
        (base_env, base_starts, base_goals) = random_fill(10, 0.5, 10, random.Random(0))
        # --------

        # everything the same
        (same_env, same_starts, same_goals) = random_fill(10, 0.5, 10, random.Random(0))
        ...
        (other_env, other_starts, other_goals) = random_fill(
    ...
    def test_generate_like_policylearn_gen_determinism(self):
        (base_env, base_starts, base_goals) = walls(10, 0.5, 10, random.Random(0))
        # --------

        # everything the same
        (same_env, same_starts, same_goals) = walls(10, 0.5, 10, random.Random(0))
        self.assertTrue(np.all(base_env == same_env))
        self.assertTrue(np.all(base_starts == same_starts))
        self.assertTrue(np.all(base_goals == same_goals))
        # --------
        ...
        # everything different
        (other_env, other_starts, other_goals) = walls(10, 0.5, 10, random.Random(1))
        self.assertFalse(np.all(base_env == other_env))
        self.assertFalse(np.all(base_starts == other_starts))
        self.assertFalse(np.all(base_goals == other_goals))
        # --------
        ...
        # only env different -> all different
        (other_env, other_starts, other_goals) = walls(10, 0.4, 10, random.Random(0))
        self.assertFalse(np.all(base_env == other_env))
        self.assertFalse(np.all(base_starts == other_starts))
        self.assertFalse(np.all(base_goals == other_goals))
    ...
    def test_generate_like_policylearn_gen_low_fills(self):
        ...
        (env, starts, goals) = walls(10, 0.1, 10, random.Random(0))
        self.assertEqual(np.count_nonzero(env), 10)  # 10% of 10*10

        (env, starts, goals) = walls(10, 0, 10, random.Random(0))
    ...
    def test_get_random_next_to_free_pose_or_any_if_full_empty(self):
        ...
        pos_empty = get_random_next_to_free_pose_or_any_if_full(env_empty, rng)
    ...
    def test_can_area_be_set_raises(self):
        ...
        self.assertRaises(AssertionError, lambda: can_area_be_set(wrong_height))

    def test_can_be_set_true(self):
        # completely free area can be set
        free = np.array([[1, 0, 0], [1, 0, 0], [1, 0, 0]])
    ...
    def test_can_be_set_false(self):
        # some two changes can not be set
        some = np.array([[1, 0, 1, 0], [1, 0, 0, 0], [1, 1, 1, 1]])
    ...
    def test_corridor_with_passing(self):
        ...
        (env, starts, goals) = corridor_with_passing(size, 0, 2, random.Random(0))
        ...
        path_len = abs(starts[0, 0] - goals[0, 0]) + abs(starts[0, 1] - goals[0, 1]) + 1
        self.assertEqual(
            np.count_nonzero(env == FREE), path_len + 1
        )  # +1 for passing point
        self.assertEqual(np.count_nonzero(env == OBSTACLE), size**2 - path_len - 1)
    ...
    def test_arena_with_crossing_no_duplicate_starts_or_goals(self):
        ...
        for _ in range(10):
            # test if correct dimensions are returned
            (env, starts, goals) = arena_with_crossing(10, 0, 8, rng)
            self.assertEqual(env.shape, (10, 10))
