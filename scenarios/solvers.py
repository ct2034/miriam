...
def cached_ecbs(
    ...
    if skip_cache:
        ...
        if storage.has_result(scenario, ResultType.ECBS_DATA, solver_params):
            data = storage.get_result(scenario, ResultType.ECBS_DATA, solver_params)
        else:
            data = ecbs(env, starts, goals, timeout=timeout)
            storage.save_result(scenario, ResultType.ECBS_DATA, solver_params, data)
    return data
...
def _ecbs_data_to_paths(data):
    ...
    for i_a in range(n_agents):
        ...
        one_path = list(map(lambda d: [d["x"], d["y"], d["t"]], schedule[key]))
...
def cached_cbsr(
    ...
    if skip_cache:
        ...
        if storage.has_result(scenario, ResultType.CBSR_PATHS, solver_params):
            paths = storage.get_result(scenario, ResultType.CBSR_PATHS, solver_params)
        else:
            paths = plan_cbsr(
                env,
                starts,
            ...
            storage.save_result(scenario, ResultType.CBSR_PATHS, solver_params, paths)
    return paths
...
def cached_icts(env, starts, goals, timeout=DEFAULT_TIMEOUT_S, skip_cache=False):
    scenario = (env, starts, goals)
    if skip_cache:
        info = icts(env, starts, goals, timeout)
    else:
        if storage.has_result(scenario, ResultType.ICTS_INFO, {}):
            info = storage.get_result(scenario, ResultType.ICTS_INFO, {})
        else:
            info = icts(env, starts, goals, timeout)
...
def cached_decentralized(
    env, starts, goals, policy, ignore_finished_agents, skip_cache=False
):
    solver_params = {
        "policy": policy.name,
        "ignore_finished_agents": ignore_finished_agents,
    ...
    scenario = (env, starts, goals)
    if skip_cache:
        metrics = decentralized(env, starts, goals, policy, ignore_finished_agents)
    else:
        if storage.has_result(scenario, ResultType.DECEN, solver_params):
            metrics = storage.get_result(scenario, ResultType.DECEN, solver_params)
        else:
            metrics = decentralized(env, starts, goals, policy, ignore_finished_agents)
            storage.save_result(scenario, ResultType.DECEN, solver_params, metrics)
    return metrics


def decentralized(env, starts, goals, policy, ignore_finished_agents):
    ...
    return run_a_scenario(
        ...
        agents,
        plot=False,
        iterator=IteratorType.LOOKAHEAD1,
        ignore_finished_agents=ignore_finished_agents,
    )
