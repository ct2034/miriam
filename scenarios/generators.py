...
GENERATOR_TYPE = Callable[
    [int, float, int, random.Random], Tuple[np.ndarray, np.ndarray, np.ndarray]
]


def make_starts_goals_on_env(env: np.ndarray, n_agents: int, rng: random.Random):
...
def random_fill(size: int, fill: float, n_agents: int, rng: random.Random):
...
def generate_walls_gridmap(width: int, height: int, fill: float, rng: random.Random):
    ...
    while np.count_nonzero(gridmap) < fill * width * height:
        ...
        start = (rng.randint(0, width - 1), rng.randint(0, height - 1))
        if direction:  # x
            gridmap[start[0] : rng.randint(0, width - 1), start[1]] = OBSTACLE
        else:  # y
            gridmap[start[0], start[1] : rng.randint(0, height - 1)] = OBSTACLE
    while np.count_nonzero(gridmap) > fill * width * height:
        make_free = (rng.randint(0, width - 1), rng.randint(0, height - 1))
        gridmap[make_free] = FREE
    return gridmap


def walls(size: int, fill: float, n_agents: int, rng: random.Random):
    env = generate_walls_gridmap(size, size, fill, rng)
...
def get_random_next_to_free_pose_or_any_if_full(env: np.ndarray, rng: random.Random):
...
def tracing_pathes_in_the_dark(
    ...
    if fill == 0:
        ...
        to_clear_start = int((1.0 - fill) * size * size)
...
def movingai_read_mapfile(mapfile: str):
    ...
    def decode(c):
        return FREE if c == FREE_CHAR else OBSTACLE
...
def movingai(map_str: str, scene_str: str, scene_nr: int, n_agents: int):
    ...
    assert (
        scene_str == "even" or scene_str == "random"
    ...
    for subdir, dirs, files in os.walk(MOVINGAI_PATH):
        for filename in files:
            ...
            if filepath.endswith(map_str + ".map"):
                mapfile = filepath
            if (
                filepath.endswith(".scen")
                and map_str in filepath
                and scene_nr_str in filepath
            ):
...
def can_area_be_set(area):
    ...
    sequence = area[[0, 0, 0, 1, 2, 2, 2, 1], [0, 1, 2, 2, 2, 1, 0, 0]]
    diffs = np.where(sequence != np.roll(sequence, 1))
    return len(diffs[0]) <= 2 and no_diagonals(area)
...
def can_be_set(env, pos):
    ...
    for x, y in product(range(3), repeat=2):
        ...
        if (
            padded_env[0] < 0
            or padded_env[0] >= env.shape[0]
            or padded_env[1] < 0
            or padded_env[1] >= env.shape[1]
        ):
            area[x, y] = OBSTACLE
        else:
...
def building_walls(size: int, fill: float, n_agents: int, rng: random.Random):
...
def corridor_with_passing(size: int, _: float, n_agents: int, rng: random.Random):
    ...
    while corridor_end[0] == corridor_start[0] or corridor_end[1] == corridor_start[1]:
        corridor_end = (rng.randrange(size), rng.randrange(size))
    # random path from start to end
    left = min(corridor_start[0], corridor_end[0])
    leftmost_pose = corridor_start if corridor_start[0] == left else corridor_end
    rightmost_pose = corridor_end if leftmost_pose == corridor_start else corridor_start
    lower = min(corridor_start[1], corridor_end[1])
    lowermost_pose = corridor_start if corridor_start[1] == lower else corridor_end
    uppermost_pose = (
        corridor_end if lowermost_pose == corridor_start else corridor_start
    )
    path_len = abs(corridor_start[0] - corridor_end[0]) + abs(
        corridor_start[1] - corridor_end[1]
    )
    assert path_len > 0
    path = [
        leftmost_pose,
    ...
    for x, y in [(0, -1), (0, 1), (-1, 0), (1, 0)]:
        if not (
            midpoint[0] + x < 0
            or midpoint[0] + x >= size
            or midpoint[1] + y < 0
            or midpoint[1] + y >= size
        ):
            if env[midpoint[0] + x, midpoint[1] + y] == OBSTACLE:
                possible_passing_points.append((midpoint[0] + x, midpoint[1] + y))
...
def arena_with_crossing(size: int, _: float, n_agents: int, rng: random.Random):
    env = np.zeros((size, size), dtype=np.int8)
    n_possible_poses = size * 2 - 2
    assert n_agents <= n_possible_poses
    choices = rng.sample(range(n_possible_poses), k=n_agents)
    starts = list(map(lambda i: a_possible_pose(size, i, n_possible_poses), choices))
