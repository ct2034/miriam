...
{
public:
  astar_goal_visitor(Vertex goal, int * number_visited)
  : m_goal(goal), number_visited(number_visited)
  {
  }
...
public:
  ...
  void append(int i);
  pytuple plan_w_number_nodes_visited(int start, int goal);
...
AstarSolver::AstarSolver(pylist pos, pylist edges)
{
  ...
  for (std::size_t j = 0; j < num_edges; ++j) {
    ...
    weightmap[e] = sqrt(
      pow(location_v[edge_v[j].first].x - location_v[edge_v[j].second].x, 2) +
      pow(location_v[edge_v[j].first].y - location_v[edge_v[j].second].y, 2));
  }
}

pytuple AstarSolver::plan_w_number_nodes_visited(int start_i, int goal_i)
{
  ...
  try {
    ...
    astar_search_tree(
      g, start, distance_heuristic<mygraph_t, cost, location *>(location_v.data(), goal),
      predecessor_map(make_iterator_property_map(p.begin(), get(vertex_index, g)))
      .distance_map(make_iterator_property_map(d.begin(), get(vertex_index, g)))
      .visitor(astar_goal_visitor<vertex_t>(goal, &number_visited)));
  ...
  return boost::python::make_tuple(shortest_path_py, number_visited);
}

pylist AstarSolver::plan(int start_i, int goal_i)
{
  pytuple res = plan_w_number_nodes_visited(start_i, goal_i);
  return static_cast<pylist>(res[0]);
}
...
BOOST_PYTHON_MODULE(libastar_graph)
{
  ...
  .def("plan_w_number_nodes_visited", &AstarSolver::plan_w_number_nodes_visited);
