...
xx, yy = np.meshgrid(
    np.linspace(-.5, grid.shape[0] + .5, grid.shape[0] * 100),
    np.linspace(-.5, grid.shape[1] + .5, grid.shape[1] * 100))
...
plt.contourf(xx, yy, img, origin=None, antialiased=False, nchunks=0, cmap=cm.Greys)
