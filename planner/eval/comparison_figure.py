...
if __name__ == "__main__":
    ...
    if data:
        ...
        keys = data.keys()
        n_results = len(
            list(filter(lambda s: s.startswith("test_planner_comparison"), keys))
        )
    ...
    labels = ["TCBS-NN2", "Greedy", "MINLP", "TPTS"]  # 1  # 4  # 2  # 3
    ...
    for i_d, i_sq in [(1, 0), (2, 2), (3, 3), (4, 1)]:
        solution_quality[i_sq, 0:n_results] = (
            results[i_d, 0, :] - results[0, 0, :]
        ) / 2  # mean per tasks
        solution_quality[i_sq, n_results : n_results * 2] = (
            results[i_d, 1, :] - results[0, 1, :]
        ) / 3
        solution_quality[i_sq, n_results * 2 : n_results * 3] = (
            results[i_d, 2, :] - results[0, 2, :]
        ) / 4
    ...
    fig.tight_layout()
    fig.subplots_adjust(left=0.1, bottom=0.15, right=0.9, top=0.9, wspace=0.1, hspace=0)
    titles = ["TCBS"] + labels
    # data -> time
    for i_d, i_t in [(0, 0), (1, 1), (2, 3), (3, 4), (4, 2)]:
        ...
        plt.setp(
            axes[i_t], xticks=[y + 1 for y in range(len(labels))], xticklabels=labels
        )
