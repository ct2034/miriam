...
if __name__ == "__main__":
    ...
    G = nx.grid_graph([n, n])
    map = np.zeros([n, n])
    map[: int(n * 0.8), int(n * 0.2)] = -1
    map[int(n * 0.8), int(n * 0.2) : int(n * 0.6)] = -1
    map[int(n * 0.2), int(n * 0.4) : int(n * 0.8)] = -1
    map[int(n * 0.2) :, int(n * 0.8)] = -1

    start = (int(n * 0.1), int(n * 0.1))
    goal = (int(n * 0.9), int(n * 0.9))
    ...
    if n < 500:
        plt.figure()
        pos = nx.spring_layout(G, iterations=1000, k=0.1 / np.sqrt(n))
        nx.draw(G, pos)
