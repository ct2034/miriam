...
#                 #    bias_regularizer='l2',
#                 input_shape=(img_len_t, img_width, img_width, img_depth_frames),
#             ),
#             BatchNormalization(),
#             Dropout(dropout),
...
#             Flatten(),
#             Dense(
#                 256, activation="relu", kernel_regularizer="l2", bias_regularizer="l2"
#             ),
#             BatchNormalization(),
#             Dropout(dropout),
#             Dense(
...
def fix_data_convrnn(images):
    return np.moveaxis(images, [1, 2, 3], [-3, -2, -4])
...
#         default="my_model.h5",
#     )
#     parser.add_argument("-t", "--model_type", choices=[CLASSIFICATION_STR, CONVRNN_STR])
...
#                 )
#                 val_images = np.array([d[i][0] for i in range(n_train + n_test, n)])
#                 val_labels = np.array([d[i][1] for i in range(n_train + n_test, n)])
#                 assert len(d) == len(train_images) + len(test_images) + len(val_images)
#             else:
#                 n_train = n - n_val
#                 print(f"n_train: {n_train}")
#                 train_images = np.array([d[i][0] for i in range(n_train)])
...
#                     model: keras.Model = keras.models.load_model(model_fname)
#                 else:
#                     print(
#                         "model does not exist. going to make a new one ... of type "
#                         + model_type
#                     )
#                     if model_type == CLASSIFICATION_STR:
...
#         one_test_loss, one_test_accuracy = model.evaluate([test_images], test_labels)
