...
def add_padding_to_gridmap(gridmap, radius):
    ...
    padded_gridmap = np.ones(
        [size[0] + 2 * radius, size[1] + 2 * radius], dtype=np.int8
    )
    padded_gridmap[radius : size[0] + radius, radius : size[1] + radius] = gridmap
    return padded_gridmap


def init_empty_fov(radius, t):
    return np.zeros([1 + 2 * radius, 1 + 2 * radius, t])
...
def make_all_agents_fovs(paths, agent, other_agent, radius):
    ...
    for i_t in range(t):
        ...
        for i_a in [i for i in range(len(paths)) if i != agent] + [
            ...
            d = paths[i_a][i_t] - pos
            if abs(d[0]) <= radius and abs(d[1]) <= radius:
                other_agent_fovs[int(d[0]) + radius, int(d[1]) + radius, i_t] = (
                    1.0 if i_a == other_agent else 0.5
                )
    ...
    for i_t_steps in range(t_until_col + 1):
        for i_a in range(len(paths)):
            ...
            for i_t_path in range(paths[i_a].shape[0]):
                ...
                if abs(d[0]) <= radius and abs(d[1]) <= radius:
                    fov_to_write[int(d[0]) + radius, int(d[1]) + radius, i_t_steps] += (
                        i_t_path + 1
                    ) / lengths[i_a]
...
def extract_all_fovs(t, paths_until_col, paths_full, padded_gridmap, i_a, i_oa, radius):
    obstacle_fovs = make_obstacle_fovs(padded_gridmap, paths_until_col[i_a], t, radius)
    pos_other_agent_fovs = make_all_agents_fovs(paths_until_col, i_a, i_oa, radius)
    (path_fovs, paths_other_agent_fovs, paths_other_agents_fovs) = make_path_fovs(
        paths_full, paths_until_col, i_a, i_oa, t, radius
    )
    x = np.stack(
        [
            obstacle_fovs,
            pos_other_agent_fovs,
            path_fovs,
            paths_other_agent_fovs,
            paths_other_agents_fovs,
        ],
        axis=3,
    )
    return x
