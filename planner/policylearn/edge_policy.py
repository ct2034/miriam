...
class EdgePolicyDataset(Dataset):
    def __init__(
        self, path, transform=None, pre_transform=None, pre_filter=None, gpu=None
    ):
    ...
    def add_file(self, fname):
        ...
        lookup_section = [(fname, i_d) for i_d in range(len_this_file)]
...
class EdgePolicyModel(nn.Module):
    def __init__(
        ...
        for i in range(num_conv_layers):
            ...
            self.conv_layers.append(
                torch_geometric.nn.ChebConv(
                    channels_in, num_conv_channels, K=cheb_filter_size
                )
            )
        ...
        for i in range(num_readout_layers):
            ...
            self.readout_layers.append(torch.nn.Linear(num_conv_channels, channels_out))
    ...
    def forward(self, x, edge_index, batch):
        ...
        for i_b in torch.unique(batch):
            y_out_batched[batch == i_b] = torch.softmax(x[batch == i_b], dim=0)
        return y_out_batched

    def predict_probablilistic(
        self, x, edge_index, big_from_small, rng=random.Random(0)
    ):
        targets, score = self.predict_scores_and_targets(x, edge_index)
        score_potential_targets = score[targets]
        # if self.training:
        node_small = rng.choices(
            targets, weights=score_potential_targets.tolist(), k=1
        )[0].item()
        # else:  # eval
    ...
    def predict_scores_and_targets(self, x, edge_index):
        ...
        n_nodes = x.shape[0]
        try:
            node = torch.nonzero(x[:, 0] == 1.0).item()
        except ValueError:
            raise RuntimeError("Unable to find own position in x")
        relevant_edge_index = edge_index[
            :, torch.bitwise_or(edge_index[0] == node, edge_index[1] == node)
        ]
    ...
    def learn(self, databatch: Data, optimizer):
        ...
        self.train()
        y_out_batched = self.forward(databatch.x, databatch.edge_index, databatch.batch)

        loss = F.binary_cross_entropy(y_out_batched, databatch.y)
        try:
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()
