...
def make_random_data_self(rng, n_nodes, num_node_features):
    fully_connected_edge_index = torch.tensor(
        [[a, b] for a, b in product(range(n_nodes), repeat=2)], dtype=torch.long
    ).T
    node = rng.choice(range(n_nodes))
    x = torch.rand(n_nodes, num_node_features) * 2.0
    x[node, 0] = 1.0
    return Data(x=x, edge_index=fully_connected_edge_index, y=node)
...
def demo_learning():
    ...
    # eval set
    eval_set = [
        make_random_data_self(rng, n_nodes, num_node_features) for _ in range(50)
    ]

    # learning to always use self edge
    optimizer = torch.optim.Adam(model.parameters(), lr=0.01)
    for i in range(int(1e3)):
        ds = [make_random_data_self(rng, n_nodes, num_node_features) for _ in range(10)]
...
def try_model_in_scenario(model):
    ...
    agents = scenarios.evaluators.to_agent_objects(
        env_g,
        starts_g,
        goals_g,
    ...
    paths: List[Any] = []
    stats = run_a_scenario(
        env, agents, plot=False, iterator=IteratorType.LOOKAHEAD3, paths_out=paths
    )
    print(stats)
    print(paths)
...
def demo_graph():
    ...
    agents = [Agent(g, 0, radius=0.1), Agent(g, 3, radius=0.1)]
    ...
    data, _, big_from_small = agents_to_data(agents=agents, i_self=0, hop_dist=2)
    print(data.x)
    pos = {i_n: g.nodes[big_from_small[i_n]][POS] for i_n in range(data.num_nodes)}
