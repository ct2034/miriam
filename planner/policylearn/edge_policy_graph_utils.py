...
def t_to_data(t: int, path_i: int) -> float:
    ...
    if t < path_i:
        ...
        return 1.0 + (t - path_i) * 0.1


def agents_to_data(
    agents, i_self: int, hop_dist: int = 3, with_optimal=True
) -> Tuple[Data, BFS_TYPE]:
    ...
    for t, p in enumerate(agents[i_self].path):
        if p in g_sml.nodes:
            ...
            if x_layer_own_path[p_sml].item() == 0.0:
        ...
        for t, p in enumerate(a.path):
            if p in g_sml.nodes:
                ...
                x_layer_other_paths[p_sml] = max(
                    float(x_layer_other_paths[p_sml].item()), t_to_data(t, a.path_i)
                )
    ...
    # 4. relative angle
    relative_angle = torch.atan2(relative_pos[:, 1], relative_pos[:, 0])
    ...
    edge_index = torch.tensor(
        [(small_from_big[n1], small_from_big[n2]) for (n1, n2) in g_sml.edges]
    ).t()
    ...
    d = Data(
        ...
        x=torch.cat(
            [
                ...
                relative_angle.view(-1, 1),
            ],
            dim=1,
        ),
        y=y,
        pos=torch.tensor(
            np.array([pos[big_from_small[i]] for i in range(len(small_from_big))])
        ),
