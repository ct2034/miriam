    ...
    def tests_optimality(self):
        ...
        for file_or_dir in os.listdir(DIRECTORY):
            ...
            if os.path.isdir(dir) and file_or_dir.startswith("optimality"):
                ...
                rng = Random(0)
                graph = nx.read_gpickle(os.path.join(
                    dir, f"{file_or_dir}.gpickle"))
                assert isinstance(graph, nx.Graph)
                model = EdgePolicyModel()
                model.load_state_dict(torch.load(os.path.join(
                    dir, f"{file_or_dir}.pt")))
                starts_goals = None
                with open(os.path.join(
                        dir, f"starts_goals.json")) as f:
                ...
                # check optimality
                timed_len_optimal = res_sim_optimal[2] + \
                    res_sim_optimal[0]*min_edge_length
                timed_len_learned = res_sim_learned[2] + \
                    res_sim_learned[0]*min_edge_length
