...
{
public:
  ImageStateValidityChecker(ompl::base::SpaceInformationPtr si, const std::vector<unsigned char>& image, unsigned width,
                            unsigned height)
...
public:
  ...
  int run(std::string environmentFile, std::string outputFile, std::string configFile)
  {
    ...
    //       "type", po::value<GenerationType::Type>(&genType)->default_value(GenerationType::SPARS)->multitoken(),
    //       "Method, one of [SPARS,SPARS2]. Default: SPARS");
    ...
    if (genType == GenerationType::SPARS || genType == GenerationType::SPARS2)
    {
      ...
      if (genType == GenerationType::SPARS)
      {
        ...
        p.constructRoadmap(base::timedPlannerTerminationCondition(node["maxTime"].as<double>()), true);
...
BOOST_PYTHON_MODULE(libsparspy)
{
  ...
  class_<Spars>("Spars", init<>()).def("run", &Spars::run);
}