...
def get_initial_configuration(
    N, random_influence=0.2, rng: Random = Random()
...
def sample_points_reaction_diffusion(
    n: int, map_img: MAP_IMG, rng: Random
    ...
    for i in range(n_searches):
        ...
        try:
            delta_t, experiment, N_simulation_steps, size = get_experiments(alpha)
            mask = cv2.resize(np.array(map_img).astype(np.float32), (size, size))
            mask = mask < 255
...
def get_experiments(alpha: float = 0.5):
    ...
    for key in ["DA", "DB"]:
        experiment[key] = experiment[key] - (alpha - 0.5) * 0.1
