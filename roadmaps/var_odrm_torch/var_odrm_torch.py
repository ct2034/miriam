...
def is_coord_free(map_img: MAP_IMG, point: Tuple[float, float]) -> bool:
    """Check if a coordinate is free."""
    size = len(map_img)
    try:
        return map_img[int(point[0] * size)][int(point[1] * size)] >= 255
    except IndexError:
        print(f"IndexError: {point}")


def is_pixel_free(map_img: MAP_IMG, point: Tuple[float, float]) -> bool:
    """Check if a pixel is free."""
    return map_img[int(point[0])][int(point[1])] >= 255


def check_edge(
    pos: Union[np.ndarray, torch.Tensor], map_img: MAP_IMG, a: int, b: int
) -> bool:
...
def sample_points(
    n: int, map_img: MAP_IMG, rng: Random, free_points: bool = True
) -> torch.Tensor:
    ...
    return torch.tensor(
        points, device=torch.device("cpu"), dtype=torch.float, requires_grad=True
    )


def make_graph_and_flann(
    pos: torch.Tensor, map_img: MAP_IMG, desired_n_nodes: int, rng: Random
) -> Tuple[nx.Graph, FLANN]:
    ...
    nx.set_node_attributes(g, {i: pos_np[i] for i in range(len(pos_np))}, POS)
    nx.set_edge_attributes(g, [], DISTANCE)
    for a, b in g.edges():  # type: ignore
        if not check_edge(pos, map_img, a, b):
            g.remove_edge(a, b)
        else:
            g.edges[(a, b)][DISTANCE] = float(np.linalg.norm(pos_np[a] - pos_np[b]))
    # add self-edges
    for node in g.nodes():
        if not g.has_edge(node, node):
            g.add_edge(node, node)
            g.edges[(node, node)][DISTANCE] = 0.0
...
def plan_path_between_nodes(g: nx.Graph, start: int, goal: int) -> Optional[List[int]]:
...
def plan_path_between_coordinates(
    g: nx.Graph, flann: FLANN, start: Tuple[float, float], goal: Tuple[float, float]
) -> Optional[PATH_W_COORDS]:
    ...
    result, _ = flann.nn_index(
        np.array([start, goal], dtype="float32"), nn, random_seed=0
    )
...
def make_paths(
    g: nx.Graph, n_paths: int, map_img: MAP_IMG, rng: Random
) -> List[PATH_W_COORDS]:
...
def get_path_len(
    pos: torch.Tensor, path: PATH_W_COORDS, training: bool
) -> torch.Tensor:
    ...
    for i in range(len(path_vs) - 1):
        j = i + 1
        sections[i] = torch.linalg.vector_norm(pos[path_vs[i]] - pos[path_vs[j]])
    s_tail = torch.linalg.vector_norm(torch.tensor(start) - pos[path_vs[0]])
    if training:
        sections[-2] = TAIL_WEIGHT * (s_tail**2 + s_tail)
    else:
        sections[-2] = s_tail
    g_tail = torch.linalg.vector_norm(torch.tensor(goal) - pos[path_vs[-1]])
    if training:
...
def get_paths_len(
    pos: torch.Tensor, paths: List[PATH_W_COORDS], training: bool
) -> torch.Tensor:
