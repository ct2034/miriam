...
def draw_graph(g, pos, paths=[]):
    ...
    for p in paths:
        for i in range(len(p)-1):
            j = i+1
            plt.plot(
                [pos_np[p[i], 0], pos_np[p[j], 0]],  # x
                [pos_np[p[i], 1], pos_np[p[j], 1]],  # y
                'y--')
...
def make_paths(g, n_paths, seed=random.randint(0, 10E6)):
    ...
    for _ in range(n_paths):
        ...
        paths.append(
            nx.shortest_path(g, start, goal, 'distance'))
    return paths
...
def get_path_len(pos, path):
    ...
    for i in range(len(path)-1):
        j = i+1
        sections[i] = torch.linalg.vector_norm(pos[path[i]] - pos[path[j]])
...
if __name__ == "__main__":
    ...
    for i_b in range(batches):
        ...
        backward = length.backward(create_graph=True)
        optimizer.step()
        g = make_graph(pos)
        if i_b % 100 == 0:
