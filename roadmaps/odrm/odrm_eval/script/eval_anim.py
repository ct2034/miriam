...
if __name__ == "__main__":
    ...
    ax = plt.axes(
        xlim=(0, 1000), ylim=(0, 1000), xticks=[], yticks=[], aspect="equal", alpha=0.5
    )
    fig.add_axes(ax)
    ...
    def animate(t):
        for lnum, line in enumerate(lines):
            line.set_data(paths_ev[t, lnum, 0], paths_ev[t, lnum, 1])
        return lines

    anim = animation.FuncAnimation(
        fig, animate, init_func=init, frames=range(T), interval=20
    )
    ...
    anim.save("iros19odrm.mp4", fps=30, extra_args=["-vcodec", "libx264"])
