...
if __name__ == "__main__":
    ...
    for agents, agent_diameter, i_trial in product(agent_ns, [10], range(1)):
        logging.info("agents: " + str(agents))
        logging.info("agent_diameter: " + str(agent_diameter))
        v = 0.2
        nn = 1
        batch = get_unique_batch(N, agents)
        cost_ev, paths_ev = eval_disc(batch, ge, posar, agent_diameter, v)
        ...
        for e in nx.edges(ge):
            g_undirected.add_edge(e[0], e[1], distance=dist(posar[e[0]], posar[e[1]]))
        cost_undirected, paths_undirected = eval_disc(
            batch, g_undirected, posar, agent_diameter, v
        )
        ...
        b = im.shape[0]
        fakenodes1 = np.array(np.array(list(product([0, b], np.linspace(0, b, 6)))))
        fakenodes2 = np.array(np.array(list(product(np.linspace(0, b, 6), [0, b]))))
        tri = Delaunay(
            np.append(posar_random, np.append(fakenodes1, fakenodes2, axis=0), axis=0)
        )
            ...
            for n in neigbours:
                if (i_n < n) & (n < N):
                    ...
                    if all([is_pixel_free(im, x) for x in line]):
                        g_random.add_edge(
                            i_n, n, distance=dist(posar_random[i_n], posar_random[n])
                        )
                        g_random.add_edge(
                            n, i_n, distance=dist(posar_random[i_n], posar_random[n])
                        )
        cost_random, paths_random = eval_disc(
            batch, g_random, posar_random, agent_diameter, v
        )
        write_csv(agents, paths_random, "random", i_trial, fname)

        logging.info(
            "our: %d, undir: %d, (our-undir)/our: %.3f%%"
            % (
                cost_ev,
                cost_undirected,
                100.0 * float(cost_ev - cost_undirected) / cost_ev,
            )
        )
        logging.info(
            "our: %d, rand: %d, (our-rand)/our: %.3f%%\n-----"
            % (cost_ev, cost_random, 100.0 * float(cost_ev - cost_random) / cost_ev)
        )

        res[agents]["undir"].append(100.0 * float(cost_ev - cost_undirected) / cost_ev)
        res[agents]["rand"].append(100.0 * float(cost_ev - cost_random) / cost_ev)
    ...
    assert is_eval_file(fname_write), (
        "Please write " "results to eval file (ending with pkl.eval)"
    )
