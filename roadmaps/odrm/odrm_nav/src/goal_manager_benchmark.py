...
SQRT2_2 = sqrt(2) / 2
PI_2 = pi / 2
GOAL_TOLERANCE_TRANS = 0.2
GOAL_TOLERANCE_ROT = 0.1
...
class GoalManager:
    def __init__(self, n_agents, poses):
        ...
        for i_a in range(n_agents):
            self.pubs.append(
                rospy.Publisher(
                    "/robot_{}/move_base_simple/goal".format(i_a),
                    PoseStamped,
                    queue_size=0,
                )
            )
        ...
        while not all(finished):
            ...
            for i_a in range(n_agents):
                trans = self.tfBuffer.lookup_transform(
                    "map", "robot_{}/base_link".format(i_a), latest
                )
                dx = abs(
                    trans.transform.translation.x - self.goals[i_a].pose.position.x
                )
                dy = abs(
                    trans.transform.translation.y - self.goals[i_a].pose.position.y
                )
                goal_yaw = euler_from_quaternion(
                    [
                        ...
                        self.goals[i_a].pose.orientation.w,
                    ]
                )[2]
                current_yaw = euler_from_quaternion(
                    [0, 0, trans.transform.rotation.z, trans.transform.rotation.w]
                )[2]
                dyaw = abs(goal_yaw - current_yaw)
                while dyaw > pi:
                    dyaw -= 2 * pi
                rospy.logdebug(
                    "i_a: {}, dx: {}, dy: {}, dyaw: {}".format(i_a, dx, dy, dyaw)
                )
                if (
                    dx < GOAL_TOLERANCE_TRANS
                    and dy < GOAL_TOLERANCE_TRANS
                    and dyaw < GOAL_TOLERANCE_ROT
                    and self.last_status[i_a] == 3
                ):
                    finished[i_a] = True
