...
def test_grad_func():
    ...
    for nn in [1, 3]:
        ...
        batch = np.array(
            [[posar[i] + .5 * random(),
              posar[j] + .5 * random()] for (i, j) in combinations(
                                                        [0, 2, 6, 8], 2
                                                     )])
        ...
        for i in range(N):
            for j in range(N):
                ...
                d[i, j] = 1
                dpp, dep, cp = grad_func(batch, nn, g, ge, posar,
                                         edgew + d * epsilon)
                dpm, dem, cm = grad_func(batch, nn, g, ge, posar,
                                         edgew - d * epsilon)
        ...
        for i in range(N):
            for j in range(2):
                ...
                d[i, j] = 1
                dpp, dep, cp = grad_func(batch, nn, g, ge,
                                         posar + d * epsilon, edgew)
                dpm, dem, cm = grad_func(batch, nn, g, ge,
                                         posar - d * epsilon, edgew)
