...
class RoadmapToTest:
    def __init__(
        self, map_fname: str, rng: Random, roadmap_specific_kwargs: Dict[str, Any] = {}
    ):
    ...
    def _plan_path_coords(
        ...
        result, _ = self.flann.nn_index(
            np.array([a, b], dtype="float32"), nn, random_seed=0
        )
    ...
    def _set_graph(self, g: nx.Graph):
        ...
        for n, (x, y) in nx.get_node_attributes(self.g, POS).items():
            self.pos_np[n, :] = [x, y]
        self.flann.build_index(np.array(self.pos_np, dtype=np.float32), random_seed=0)
    ...
    def evaluate_path_length(self) -> Dict[str, float]:
        ...
        for i in range(self.n_eval):
            ...
            while not is_coord_free(map_img_inv, a) or not is_coord_free(
                map_img_inv, b
            ):
                a = self.rng.random(), self.rng.random()
                b = self.rng.random(), self.rng.random()
            ...
            if path_w_coords[-1] is None:
                ...
                a, pos_t[path_w_coords[-1][0]].tolist(), map_img_inv
            ):
                success = False
            elif not self._check_line(
                b, pos_t[path_w_coords[-1][-1]].tolist(), map_img_inv
            ):
                success = False
            data[f"success_{i:03d}"] = success
            if success:
                data[f"path_length_{i:03d}"] = get_path_len(
                    pos_t, path_w_coords, False
                ).item()
        ...
        for i in range(self.n_eval):
            ...
            while not found:
                ...
                if straight_len < 0.5:
                    continue
                try:
                    path = plan_path_between_coordinates(self.g, flann, start, end)
                except Exception:
    ...
    def plot_example(self, folder: str, i: int):
        ...
        pos = {
            k: (v[0] * len(self.map_img), v[1] * len(self.map_img))
        ...
        while path is None:
            try:
                ...
                path_len = get_path_len(pos_torch, path, False).item()
                a, b, _ = path
                straight_len = np.linalg.norm(np.array(a) - np.array(b)).item()
                if straight_len < 0.3:
        ...
        full_path = (
            [(start[0] * len(self.map_img), start[1] * len(self.map_img))]
            + coord_path
            + [(end[0] * len(self.map_img), end[1] * len(self.map_img))]
        )
        x, y = zip(*full_path)
        ax.plot(x, y, color="red", linewidth=0.8, alpha=0.7)

        # what will be the file name
        map_no_ext = os.path.splitext(os.path.basename(self.map_fname))[0]

        # save
        for extenstion in ["pdf", "png"]:
            name = (
                f"{i:03d}_{self.__class__.__name__}_"
...
class GSRM(RoadmapToTest):
    ...
    def __init__(self, map_fname: str, rng: Random, roadmap_specific_kwargs):
        ...
        if os.path.exists(inflated_map_fname):
            ...
            elm = cv.getStructuringElement(
                cv.MORPH_ELLIPSE, (agent_radius_map, agent_radius_map)
            )
        ...
        nx.set_node_attributes(
            g, {i: (p[1], p[0]) for i, p in nx.get_node_attributes(g, POS).items()}, POS
        )
        self._set_graph(g)
...
class SPARS2(RoadmapToTest):
    ...
    def __init__(self, map_fname: str, rng: Random, roadmap_specific_kwargs):
        ...
        dense_to_sparse_multiplier = spars_kwargs.pop("dense_to_sparse_multiplier")
        ...
        dense_deltas, sparse_deltas, stretch_factors, ns = SPARS2.size_stats[map_fname][
            target_n
        ]
        dense_delta = 1.5
        sparse_delta = dense_delta * dense_to_sparse_multiplier
        ...
        while not _goal_check(target_n, n_nodes):
            ...
            print(f"Trying denseDelta={dense_delta}, " f"sparseDelta={sparse_delta}...")
            ...
            for a, ax, ay, b, bx, by in edges:
                ...
                g.add_edge(a, b, **{DISTANCE: np.sqrt((ax - bx) ** 2 + (ay - by) ** 2)})
        ...
        for x, y in product(range(n_side), range(n_side)):
            ...
            if is_coord_free(self.map_img, coords):
                ...
                if (
                    x > 0
                    and grid[x - 1, y] != -1
                    and self._check_line(coords, g.nodes[grid[x - 1, y]][POS])
                ):
                    g.add_edge(i_to_add, grid[x - 1, y], **{DISTANCE: edge_length})
                if (
                    y > 0
                    and grid[x, y - 1] != -1
                    and self._check_line(coords, g.nodes[grid[x, y - 1]][POS])
                ):
                    g.add_edge(i_to_add, grid[x, y - 1], **{DISTANCE: edge_length})
        # swap x and y
        nx.set_node_attributes(
            g, {i: (p[1], p[0]) for i, p in nx.get_node_attributes(g, POS).items()}, POS
        )
            ...
            if is_coord_free(self.map_img, coords):
                ...
                for dx, dy in deltas:
                    ...
                    if grid[other_x, other_y] != -1 and self._check_line(
                        coords, g.nodes[grid[other_x, other_y]][POS]
                    ):
                        g.add_edge(
                            i_to_add,
...
def run():
    ...
    for i_p, ptr in enumerate(tqdm(params_to_run)):
        ...
        if _cls == PRM:
            ptr[1]["n_edges"] = df[df["roadmap"] == GSRM.__name__][
                df["map"] == map_name
            ][df["GSRM_target_n"] == target_n]["n_edges"].values[0]
        if _cls != GSRM:
            data_target_ns = df[df["roadmap"] == GSRM.__name__][df["map"] == map_name][
                df["target_n"] == target_n
            ]
            # assume that gsrm was already run
            assert (
                len(data_target_ns) == N_SEEDS
            ), f"Run gsrm first for {map_name=}, {args['target_n']=}"
...
def _run_proxy(args):
    ...
    if "plot" in args and args["plot"] and seed == 0 and map_name == PLOT_GSRM_ON_MAP:
...
def prepare_mean():
    ...
    target_ns_in_data = target_ns_in_data[np.logical_not(np.isnan(target_ns_in_data))]
    ...
    for map_, roadmap, target_n, seed in product(
        ...
        mask_this_data = np.logical_and(
            ...
            np.logical_and(
                df.seed == seed,
                np.logical_and(df.target_n == target_n, df.roadmap == roadmap),
            ),
        )
        ...
        mask_all_roadmaps = np.logical_and(
            df.map == map_, np.logical_and(df.seed == seed, df.target_n == target_n)
        )
        success_cols = _get_cols_by_prefix(df, "success_")
        # Filter out colums where all roadmaps where successful:
        success_cols = [col for col in success_cols if df[mask_all_roadmaps][col].all()]
        if len(success_cols) == 0:
            continue
        success_idx = [int(col.split("_")[-1]) for col in success_cols]
        df.at[this_i, "path_length_mean"] = (
            df[mask_this_data][[f"path_length_{i:03d}" for i in success_idx]]
            .mean(axis=1)
            .values[0]
        )
        ...
        #     ]].mean(axis=1)
        df.at[this_i, "visited_nodes_mean"] = (
            df[mask_this_data][[f"visited_nodes_{i:03d}" for i in success_idx]]
            .mean(axis=1)
            .values[0]
        )
        df.at[this_i, "success_rate"] = (
            df[mask_this_data][_get_cols_by_prefix(df, "success_")]
            .mean(axis=1)
            .values[0]
        )
...
def plot():
    ...
    plt.savefig(
        os.path.join(PLOT_FOLDER, os.path.basename(CSV_PATH).replace(".csv", ".png"))
    )
    plt.close("all")
    ...
    for roadmap in df.roadmap.unique():
        ...
        sns.pairplot(
            df_roadmap,
            hue="map",
            x_vars=[col for col in df_roadmap.columns if col.startswith(roadmap)],
    ...
    COMPARE_TO = "GSRM"
    assert (
        COMPARE_TO in df.roadmap.unique()
    ...
    for roadmap in df.roadmap.unique():
        ...
        assert len(data_len) == len(
            map_names_len
        ), f"{len(data_len)=} != {len(map_names_len)=}"
        assert len(data_rel) == len(
            map_names_rel
        ...
        for i, (d, mns) in enumerate(
            ...
            axs[i].set_xlim(min(d[:, 0]) - 0.1, max(d[:, 0]) + 0.1)
            axs[i].set_ylim(min(d[:, 1]) - 0.1, max(d[:, 1]) + 0.1)
...
def plots_for_paper():
    ...
    interesting_maps = ["plain", "den520", "rooms_30", "slam"]
    ...
    df = df[np.logical_or(df.roadmap != "SPARS2", df.target_n != min_n)]
    ...
    for key, title in [
        ...
        fig, axs = plt.subplots(2, n_plots // 2, figsize=(4.5 * (n_plots // 2), 7))
        axs = axs.flatten()
    ...
    for key, title in [
        ...
        for map_, roadmap, target_n, seed in product(
            ...
            mask_gsrm = np.logical_and(
                ...
                np.logical_and(
                    df.seed == seed,
                    np.logical_and(df.target_n == target_n, df.roadmap == "GSRM"),
                ),
            )
            ...
            success_cols = _get_cols_by_prefix(df, "success_")
            # Filter out colums where all roadmaps where successful:
            success_cols = [col for col in success_cols if df[mask_both][col].all()]
            if len(success_cols) == 0:
                continue
            success_idx = [int(col.split("_")[-1]) for col in success_cols]
            # calculate regret
            plot_df.at[this_i, f"{key}_regret"] = (
                df[mask_this_data][[f"{key}_{i:03d}" for i in success_idx]]
                .mean(axis=1)
                .values[0]
                - df[mask_gsrm][[f"{key}_{i:03d}" for i in success_idx]]
                .mean(axis=1)
                .values[0]
            ) / df[mask_gsrm][[f"{key}_{i:03d}" for i in success_idx]].mean(
                axis=1
            ).values[
                0
            ]
        ...
        for extension in ["png", "pdf"]:
            plt.savefig(
                os.path.join(
                    PLOT_FOLDER_PAPER,
                    os.path.basename(CSV_PATH).replace(
                        ".csv", f"_paper_{key}_regret.{extension}"
                    ),
                ),
                dpi=DPI,
            )
...
def table_for_paper():
    ...
    for i_m, map_name in enumerate(interesting_maps):
        ...
        mask_gsrm = np.logical_and(df.map == map_name, df.roadmap == "GSRM")
        for i_r, roadmap in enumerate(df.roadmap.unique()):
            if roadmap == "GSRM":
                continue
            mask_rm = np.logical_and(df.map == map_name, df.roadmap == roadmap)
